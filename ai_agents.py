...
def _build_agents(cfg: Dict[str, Any]):
    ...
    growth_agent = Agent(
        ...
        goal=(
            "Писать ясные, фактурные аналитические тексты, где цифры объясняются, "
            "а не продаются. Нейтрально сравнивать разные способы размещения "
            "капитала (банковский вклад, инфляция, DeFi-доходность), показывая и "
            "плюсы, и риски каждого варианта, без выводов «что лучше»."
        ),
        backstory=(
            "Ты — экс-редактор Банки.ру, ушедший в Web3. Ты презираешь "
            "корпоративную воду и ИИ-штампы. Тебе СТРОГО запрещено использовать "
            f"фразы: {cliches}. Ты пишешь спокойным аналитическим тоном без "
            "запугивания, давления и обещаний. Любую доходность (APY) ты всегда "
            "помечаешь как переменную и не гарантированную, а сравнения подаёшь "
            "как иллюстрацию, а не как рекомендацию действовать."
        ),
        llm=growth_llm,
        allow_delegation=False,
        verbose=False,
    ...
    compliance_agent = Agent(
        ...
        goal=(
            ...
            "побуждения к инвестициям, убирать запугивание и обещания дохода, "
            "приводить любую доходность (APY) к формулировке «переменная и не "
            "гарантированная», снимать оценочные сравнения в духе «X лучше Y», "
            "подтверждать некостодиальный статус TJT и встраивать предупреждения "
            "о рисках смарт-контрактов."
        ),
        backstory=(
    ...
    try:
        ...
        task_growth = Task(
            description=(
                ...
                f"токена {token} (900-1300 слов) в формате Markdown. Нейтрально и "
                "на цифрах сравни варианты размещения капитала: банковский "
                f"депозит ~{b['bank_deposit_apr']}% APR, ориентир инфляции "
                f"~{b['real_inflation_rate']}% и наблюдаемую DeFi-доходность "
                f"~{b['web3_aggregator_apy']}% APY. Все ставки подавай как "
                "иллюстративные, переменные и не гарантированные, обязательно "
                "указывая риски DeFi (уязвимости смарт-контрактов, волатильность, "
                "изменение ставок). Спокойный аналитический тон, без давления, "
                "запугивания и вывода «что выбрать». Никаких ИИ-штампов."
            ),
    ...
    try:
        ...
        task_video = Task(
            description=(
                ...
                "Shorts/TikTok с посекундной раскадровкой в образовательном ключе:\n"
                "- 0-5 сек: Нейтральный вопрос-зацепка по теме доходности.\n"
                "- 5-15 сек: Как работает банковский вклад и что такое инфляция "
                "(факты, без запугивания).\n"
                "- 15-25 сек: Что такое некостодиальные DeFi-пулы и их риски; APY "
                "переменный и не гарантированный.\n"
                "- 25-30 сек: Мягкое приглашение изучить сравнение на TJT + фраза, "
                "что это не инвестиционная рекомендация.\n"
                "Для каждого блока укажи [VISUAL] и [VOICEOVER]. Без давления, "
                "обещаний дохода и ИИ-штампов."
            ),
...
def _demo_seo_article(token: str, cfg: Dict[str, Any]) -> str:
    ...
    return f"""# {token}: как сравнивать банковский вклад, инфляцию и DeFi-доходность

**Meta:** Разбираем на иллюстративных цифрах, как соотносятся классический
депозит, инфляция и доходность некостодиальных DeFi-пулов — и какие риски есть
у каждого варианта.

## Точка отсчёта: ${capital:,.0f} на банковском депозите

При ставке **{b['bank_deposit_apr']}% APR** годовой номинальный результат — около
**${bank_gain:,.0f}**. Это предсказуемая величина, но её стоит рассматривать
вместе с инфляцией, чтобы видеть реальную, а не номинальную картину.

## Инфляция как контекст

Если ориентироваться на инфляцию около **{b['real_inflation_rate']}% в год**, то
покупательная способность тех же ${capital:,.0f} снижается примерно на
**${inflation_loss:,.0f}**. Это касается любых сбережений в фиате и не является
особенностью конкретно вклада — просто фактор, который важно учитывать.

## Для сравнения: некостодиальные DeFi-пулы

Наблюдаемая доходность DeFi-пулов может составлять порядка
**{b['web3_aggregator_apy']}% APY** — в этом примере это около **${web3_gain:,.0f}**
за год. Важно: **APY переменный и не гарантирован**, он меняется вместе со спросом
и часто зависит от временных токен-наград. Разница с банком в этом сценарии —
около **${missed:,.0f}**, но она сопровождается принципиально другими рисками
(см. ниже) и не является обещанием результата.
...
индивидуальной инвестиционной рекомендацией. Приведённые ставки иллюстративны,
переменны и не гарантированы. Взаимодействие со смарт-контрактами сопряжено с
рисками (уязвимости контрактов, волатильность, потеря капитала) и осуществляется
...
**[0-5 сек] ЗАЦЕПКА**
- [VISUAL] Спокойная графика: вклад, инфляция, DeFi-пул — три иконки.
- [VOICEOVER] «Три способа хранить капитал. Разберём, чем они отличаются.»

**[5-15 сек] ВКЛАД И ИНФЛЯЦИЯ**
- [VISUAL] График: {b['bank_deposit_apr']}% по вкладу и ориентир инфляции {b['real_inflation_rate']}%.
- [VOICEOVER] «Вклад даёт около {b['bank_deposit_apr']}% в год — предсказуемо.
  Инфляцию (~{b['real_inflation_rate']}%) стоит учитывать при любом способе хранения.»

**[15-25 сек] DEFI-ПУЛЫ И ИХ РИСКИ**
- [VISUAL] Дашборд TJT, тикер {token}, индикатор {b['web3_aggregator_apy']}% APY с пометкой «переменный».
- [VOICEOVER] «Некостодиальные пулы могут показывать порядка {b['web3_aggregator_apy']}% APY,
  но ставка переменная, не гарантирована и несёт риски смарт-контрактов.»

**[25-30 сек] МЯГКОЕ ПРИГЛАШЕНИЕ**
- [VISUAL] Логотип TJT, ссылка на сравнение.
- [VOICEOVER] «Сравнить варианты и почитать о рисках можно на TJT. Это не инвестиционная рекомендация.»

---
_Информационный материал. Ставки переменны и не гарантированы. Не является инвестиционной рекомендацией._
"""
